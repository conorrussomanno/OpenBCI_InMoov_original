...
//openBCI data packet
final int nDataBackBuff = 3*(int)openBCI.fs_Hz; //3*250 = 750 by default
...
public void serialEvent(Serial port) {
  ...
  if (port == serial_openBCI) {
    ...
    //used to determine if the console should echo the bytes read on the serial port ... you want this to be false for streamRawData mode
    if(openBCI.isStateNormal() != true){ 
      echoBytes = true;
    } else{
      echoBytes = false;
    }
